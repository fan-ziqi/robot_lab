...
def visualize_motion(npz_file):
    ...
    if "body_positions" in data and "pelvis" in body_names:
        ...
        # X component
        fig.add_trace(
            go.Scatter(x=time, y=root_positions[:, 0], name="Pelvis Position (X)", mode="lines", line=dict(width=1)),
            row=1,
            col=1,
        )
        # Y component
        fig.add_trace(
            go.Scatter(x=time, y=root_positions[:, 1], name="Pelvis Position (Y)", mode="lines", line=dict(width=1)),
            row=1,
            col=1,
        )
        # Z component
        fig.add_trace(
            go.Scatter(x=time, y=root_positions[:, 2], name="Pelvis Position (Z)", mode="lines", line=dict(width=1)),
            row=1,
            col=1,
        )
    ...
    if "dof_positions" in data:
        for i in range(len(dof_names)):
            fig.add_trace(
                go.Scatter(x=time, y=data["dof_positions"][:, i], name=dof_names[i], mode="lines", line=dict(width=1)),
                row=2,
                col=1,
            )
    ...
    if "dof_velocities" in data:
        for i in range(len(dof_names)):
            fig.add_trace(
                go.Scatter(x=time, y=data["dof_velocities"][:, i], name=dof_names[i], mode="lines", line=dict(width=1)),
                row=3,
                col=1,
            )
    ...
    if "body_linear_velocities" in data:
        for i in range(len(body_names)):
            ...
            # X component
            fig.add_trace(
                go.Scatter(x=time, y=velocities[:, 0], name=f"{body_names[i]} (X)", mode="lines", line=dict(width=1)),
                row=4,
                col=1,
            )
            # Y component
            fig.add_trace(
                go.Scatter(x=time, y=velocities[:, 1], name=f"{body_names[i]} (Y)", mode="lines", line=dict(width=1)),
                row=4,
                col=1,
            )
            # Z component
            fig.add_trace(
                go.Scatter(x=time, y=velocities[:, 2], name=f"{body_names[i]} (Z)", mode="lines", line=dict(width=1)),
                row=4,
                col=1,
            )
