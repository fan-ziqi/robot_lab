...
def plot_pelvis_data_2d(source_data, target_data, source_pelvis_idx, target_pelvis_idx):
    ...
    fig = make_subplots(
        rows=len(plots_to_create), cols=1, subplot_titles=[p[1] for p in plots_to_create], vertical_spacing=0.08
    )
    ...
    fig.update_layout(
        title_text="Pelvis Data 2D Comparison", height=350 * len(plots_to_create), legend_tracegroupgap=250
    )
    ...
    for data, idx, name in [(source_data, source_pelvis_idx, "Source"), (target_data, target_pelvis_idx, "Target")]:
        ...
        fig.add_trace(
            go.Cone(
                ...
                y=positions[::step, 1],
                z=positions[::step, 2],
                u=forward_vectors[::step, 0],
                v=forward_vectors[::step, 1],
                w=forward_vectors[::step, 2],
                sizemode="absolute",
                sizeref=0.1,
                showscale=False,
...
def update_pelvis_data(source_file, target_file, dry_run=False):
    ...
    try:
        ...
        if num_frames_source < num_frames_target:
            print(
                f"Warning: Source file has fewer frames ({num_frames_source}) than target file ({num_frames_target})."
                " Target will be truncated."
            )
            ...
            if num_frames_source > num_frames_target:
                print(
                    f"Warning: Source file has more frames ({num_frames_source}) than target file"
                    f" ({num_frames_target}). Source data will be truncated."
                )

        # Copy the pelvis data from source to target
        print("Copying pelvis data...")
        target_data_dict["body_positions"][:, target_pelvis_idx] = source_data["body_positions"][
            :num_frames, source_pelvis_idx
        ]
        target_data_dict["body_rotations"][:, target_pelvis_idx] = source_data["body_rotations"][
            :num_frames, source_pelvis_idx
        ]
        ...
        np.savez(target_file, **target_data_dict)
