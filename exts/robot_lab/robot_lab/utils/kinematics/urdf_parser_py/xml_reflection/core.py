...
class Param:
    ...
    def set_default(self, obj):
        if self.required:
            raise Exception(f"Required {self.type} not set in XML: {self.xml_var}")  # noqa
...
class Attribute(Param):
    ...
    def add_to_xml(self, obj, node):
        ...
        if value is None:
            if self.required:
                raise Exception(f"Required attribute not set in object: {self.var}")  # noqa
            elif not skip_default:
                value = self.default
...
class Element(Param):
    ...
    def add_to_xml(self, obj, parent):
        ...
        if value is None:
            if self.required:
                raise Exception(f"Required element not defined in object: {self.var}")  # noqa
            elif not skip_default:
                value = self.default
        if value is not None:
            ...
            if element is not None:
                ...
                if element.is_aggregate:
                    ...
                    if tag in unset_scalars:
                        ...
                        on_error(f"Scalar element defined multiple times: {tag}")  # noqa
